...
class ApiService
{
    ...
    // domain
    const SMS_DOMAIN = 'http://api.message.net.tw/send.php'; // 簡訊

    protected $apiRepository;

    // api 列表
    protected $apiList = [
        'login' => self::SMS_DOMAIN // 登入 發送手機號碼驗證碼
    ];
    ...
    {
        // 檢查網址是否存在
        if (isset($this->apiList[$url])) {
            return $this->apiRepository->curl(self::SMS_DOMAIN, $method, $param, $header);
        }
